...
const SplitConfirmacaoModal: React.FC<{
  ...
  const origemCliente = clientes.find(
    (cliente) => cliente.id === item.cliente?.id,
  );

  const nameOfAllClientesFromClientesIds = clientes
    .filter((cliente) => clientesIds.includes(cliente.id))
    .map((cliente) => cliente.nome)
    .filter((cliente) => cliente !== origemCliente?.nome);
    ...
    <AlertDialog open={isOpen}>
      <AlertDialogContent>
        ...
        <p className='text-[16px] font-[500] text-[#000000] leading-[16px]'>
          ...
          <br />
          <br />
          {nameOfAllClientesFromClientesIds.join(', ')} deverão aceitar sua
          solicitação de divisão para finalizar o procedimento.
        </p>
        <div className='flex flex-col justify-end gap-2'>
          <Button
            variant='tertiary'
            onClick={onConfirm}
            text={'De acordo, dividir.'}
          />
